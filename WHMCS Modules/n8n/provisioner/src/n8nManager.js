export class N8nManager {
  ...
  async deploy(payload, externalId, infra) {
    ...
    const version = payload.version_channel || 'stable'
  ...
  async waitForHealth(instanceUrl) {
    ...
    throw new Error(`n8n health check timed out for ${instanceUrl}`)
  }
  ...
  async checkHealth(url) {
    try {
      ...
      if (json.status && String(json.status).toLowerCase() !== 'ok') {
        return false
      }
  ...
  async callHook(url, token, payload) {
    ...
    if (!response.ok) {
      throw new Error(`n8n hook failed (${response.status}): ${raw}`)
    }
