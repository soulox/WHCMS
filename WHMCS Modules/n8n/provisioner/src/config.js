...
export const config = {
  ...
  n8n: {
    publicBaseDomain: process.env.N8N_PUBLIC_BASE_DOMAIN || '',
    defaultScheme: process.env.N8N_DEFAULT_SCHEME || 'https',
    deployHookUrl: process.env.N8N_DEPLOY_HOOK_URL || '',
    deployHookToken: process.env.N8N_DEPLOY_HOOK_TOKEN || '',
    limitsHookUrl: process.env.N8N_LIMITS_HOOK_URL || '',
    limitsHookToken: process.env.N8N_LIMITS_HOOK_TOKEN || '',
    backupHookUrl: process.env.N8N_BACKUP_HOOK_URL || '',
    backupHookToken: process.env.N8N_BACKUP_HOOK_TOKEN || '',
    healthCheckPath: process.env.N8N_HEALTH_CHECK_PATH || '/healthz',
    deployTimeoutMs: Number(process.env.N8N_DEPLOY_TIMEOUT_MS || 300000),
    deployPollMs: Number(process.env.N8N_DEPLOY_POLL_MS || 5000)
  }
}
...
export function validateConfig() {
  ...
  if (!config.n8n.publicBaseDomain) missing.push('N8N_PUBLIC_BASE_DOMAIN')
  if (missing.length) {
